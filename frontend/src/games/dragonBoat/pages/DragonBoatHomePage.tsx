...
export default function DragonBoatHomePage() {
  ...
  async function handleStart() {
    ...
    try {
      await Promise.all([createDragonBoatUser(name1), createDragonBoatUser(name2)])
      navigate('/games/dragon-boat/play', { state: { p1: name1, p2: name2 } })
    } catch {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', minHeight: '100vh', gap: 24, paddingTop: 48 }}>
      ...
      <p style={{ color: '#a9bfd7' }}>2-player local race. First to the finish wins.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: 320 }}>
        <label>Player 1 name (Arrow Left / Arrow Right)</label>
        <input value={p1} onChange={e => setP1(e.target.value)} placeholder="e.g. Alice" maxLength={20} />
        <label>Player 2 name (A / D)</label>
